...
const PartyBadge: React.FC<{ party: string }> = ({ party }) => {
  ...
  return (
    <span className={`inline-block px-2 py-1 text-xs text-white border-2 border-blue-950 ${colorClass}`}>
      {party}
    </span>
  );
};
...
const CandidatePortrait: React.FC<{ candidate: Candidate }> = ({ candidate }) => {
  return (
...
const ConstituencyCard: React.FC<{ code: string; name: string; candidates: Candidate[] }> = ({ code, name, candidates }) => {
  return (
...
export default function ConstituenciesPage() {
  ...
  const filteredData = sabahElectionData.filter(constituency => {
    ...
    return constituency.name.toLowerCase().includes(searchTermLower) ||
           constituency.code.toLowerCase().includes(searchTermLower) ||
           constituency.candidates.some(c => c.name.toLowerCase().includes(searchTermLower));
  });
        ...
        <main className="flex-grow container mx-auto px-4 py-12">
          <div className='text-center'>
             <h2
              ...
              <p className="text-xl text-white mb-10" style={{ textShadow: '2px 2px 0px #172554' }}>
          ...
          {filteredData.length > 0 ? (
            ...
            <div className="text-center bg-blue-900 border-4 border-dashed border-red-500 p-10 max-w-2xl mx-auto shadow-[8px_8px_0px_#172554]">
                <p className="text-2xl text-red-500" style={{ textShadow: '2px 2px 0px #172554' }}>
